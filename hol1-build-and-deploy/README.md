...
> Spin commands are tweaked for productivity and are provide all sorts of flags to fully automate them when executed in an unattended environment. That said, you could also use the `spin new -t http-rust -a hello-akamai-functions` (or respectively `spin new -t http-ts --value http-router=hono -a hello-akamai-functions`) command for creating the application with the same configuration

### Step 2 Compile the source code down to WebAssembly with `spin build`

Move into the application directory `cd hello-akamai-functions` and execute `spin build`.

```bash
spin build

Building component hello-akamai-functions
# ...
...
Available Routes:
  hello-akamai-functions: http://127.0.0.1:3000 (wildcard)
```
...
### Step 4: Deploy the application to Akamai Functions using `spin aka deploy`

#### Authenticating

As of now, Akamai Functions provides a gated onboarding process. Your workshop instructor should already have asked for your GitHub username. If not, please let your instructor immediately know. Alternatively, you can fill the following form to get an account provisioned: [Get an Akamai Functions Account](https://fibsu0jcu2g.typeform.com/fwf-preview).

Once your account has been provisioned (you'll receive a confirmation by mail), you can authenticate using the `spin aka login` command. The command will trigger a browser based authentication flow asking you for permission to interact with Akamai Functions on your behalf. Grant Spin permission for doing so.
...
Once authenticated, you can deploy, browse, inspect, observe and remove Spin applications from your individual Akamai Functions account. Let's finally deploy the `hello-akamai-functions` application:

```bash
spin aka deploy
Name of new app: hello-akamai-functions # confirm with RETURN
...
When deploying for the first time, `spin aka deploy` command asks about the desired name and for confirmation to actually deploy. Again, there are flags to deploy without the CLI asking those questions: `spin aka deploy --create-name hello-akamai-functions --no-confirm`.

**Deploying an application to Akamai Functions** takes roughly 50 seconds in which your application is uploaded to Akamai Functions, its deployed throughout all service regions across the globe and a random endpoint is generated for your workload. 🚀

```bash
spin aka deploy --no-confirm --create-name hello-akamai-functions
```

<details>
<summary>Expected Output</summary>

The `spin aka deploy --no-confirm --create-name hello-akamai-functions` command should generate an output similar to this:
...
App Routes:
- hello-akamai-functions: https://10663d58-fea4-437f-a323-6e493caff337.fwf.app (wildcard)
```

</details>

Once the command has finished, you can sent another HTTP `GET` request. However, this time we'll sent it to the endpoint generated for your application on top of Akamai Functions:
